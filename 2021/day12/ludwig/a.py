...
def explore_paths(paths, node_idx, end_idx, adj_mat, is_big):
    update_paths = []
    copy_paths = []
    if not paths:
        ...
        for i in range(0, adj_mat.shape[0]):
            if adj_mat[node_idx][i] == 1:
                if not is_big[i]:
                    for p in copy_paths:
                        if not i in p:
                            update_paths.extend(explore_paths(copy_paths, i, end_idx, adj_mat, is_big))
                else:
                    update_paths.extend(explore_paths(copy_paths, i, end_idx, adj_mat, is_big))
    else:
...
with open("input.txt", 'r') as fh:
    ...
    valid_paths = explore_paths([], node_id['start'], node_id['end'], adjacency_matrix, is_big)
