...
with open("input.txt", 'r') as fh:
    ...
    for i in range(0, 4):
        ...
        tmp_field[tmp_field > 9] = 1
        horizontal.append(np.array(tmp_field))
    tmp_field = np.hstack(horizontal)
    vertical = [tmp_field]
    for i in range(0, 4):
        tmp_field = tmp_field + 1
        tmp_field[tmp_field > 9] = 1
        vertical.append(tmp_field)
    big_field = np.vstack(vertical)
    print(big_field, big_field.shape)
    goal_point = (big_field.shape[0] - 1, big_field.shape[1] -1)
    path = dijkstra_search((0, 0), goal_point, big_field)
    print(cost_path(path, (0, 0), goal_point, big_field))
