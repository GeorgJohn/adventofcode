...
def draw_line(line, field, x_max):
    if line[1] == line[3]:
        ...
        draw_diagonal_line(line, field, x_max)
...
def draw_diagonal_line(line, field, x_max):
    if line[0] < line[2]:
        rx = range(line[0], line[2] + 1)
    else:
        rx = range(line[0], line[2] - 1, -1)
    if line[1] < line[3]:
        ry = range(line[1], line[3] + 1)
    else:
        ry = range(line[1], line[3] - 1, -1)
